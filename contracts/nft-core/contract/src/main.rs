...
use crate::{
	constants::{
		ACCESS_KEY_NAME_1_0_0,
		ACCESS_KEY_NAME_PREFIX,
		ALLOW_MINTING,
...
pub extern "C" fn init() {
	...
	let collection_name: String = utils
	...
	let collection_symbol: String = utils
		::get_named_arg_with_user_errors(
			ARG_COLLECTION_SYMBOL,
	...
	let total_token_supply: u64 = utils
		::get_named_arg_with_user_errors(
			ARG_TOTAL_TOKEN_SUPPLY,
	...
	let minting_mode: MintingMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_MINTING_MODE,
	...
	let ownership_mode: OwnershipMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_OWNERSHIP_MODE,
	...
	let holder_mode: NFTHolderMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_HOLDER_MODE,
	...
	let whitelist_mode: WhitelistMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_WHITELIST_MODE,
	...
	let contract_whitelist = utils
		::get_named_arg_with_user_errors::<Vec<ContractHash>>(
			ARG_CONTRACT_WHITELIST,
	...
	let package_hash = utils
		::get_named_arg_with_user_errors::<String>(
			ARG_NFT_PACKAGE_HASH,
	...
	let nft_metadata_kind: NFTMetadataKind = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_NFT_METADATA_KIND,
	...
	if let NFTMetadataKind::CustomValidated = nft_metadata_kind {
		casper_serde_json_wasm
			::from_str::<CustomMetadataSchema>(&json_schema)
			.map_err(|_| NFTCoreError::InvalidJsonSchema)
			.unwrap_or_revert();
	}

	let identifier_mode: NFTIdentifierMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_IDENTIFIER_MODE,
	...
	let metadata_mutability: MetadataMutability = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_METADATA_MUTABILITY,
	...
	let reporting_mode: OwnerReverseLookupMode = utils
		::get_named_arg_with_user_errors::<u8>(
			ARG_OWNER_LOOKUP_MODE,
	...
	runtime::put_key(COLLECTION_NAME, storage::new_uref(collection_name.clone()).into());
	runtime::put_key(COLLECTION_SYMBOL, storage::new_uref(collection_symbol).into());
	runtime::put_key(TOTAL_TOKEN_SUPPLY, storage::new_uref(total_token_supply).into());
	runtime::put_key(OWNERSHIP_MODE, storage::new_uref(ownership_mode as u8).into());
	...
	runtime::put_key(WHITELIST_MODE, storage::new_uref(whitelist_mode as u8).into());
	runtime::put_key(CONTRACT_WHITELIST, storage::new_uref(contract_whitelist).into());
	...
	runtime::put_key(IDENTIFIER_MODE, storage::new_uref(identifier_mode as u8).into());
	runtime::put_key(METADATA_MUTABILITY, storage::new_uref(metadata_mutability as u8).into());
	runtime::put_key(BURN_MODE, storage::new_uref(burn_mode as u8).into());
	runtime::put_key(REPORTING_MODE, storage::new_uref(reporting_mode.clone() as u8).into());
	...
	storage
		::new_dictionary(TOKEN_OWNERS)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(TOKEN_ISSUERS)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(OWNED_TOKENS)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage::new_dictionary(OPERATOR).unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(BURNT_TOKENS)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(TOKEN_COUNTS)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(METADATA_CUSTOM_VALIDATED)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(METADATA_CEP78)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(METADATA_NFT721)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(METADATA_RAW)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(HASH_BY_INDEX)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage
		::new_dictionary(INDEX_BY_HASH)
		.unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
	storage::new_dictionary(PAGE_TABLE).unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
...
pub extern "C" fn set_variables() {
	...
	if
		let Some(allow_minting) = utils::get_optional_named_arg_with_user_errors::<bool>(
			ARG_ALLOW_MINTING,
	...
	if
		let Some(new_contract_whitelist) =
...
pub extern "C" fn mint() {
	...
	let token_owner_key: Key = if
		let OwnershipMode::Assigned | OwnershipMode::Transferable = ownership_mode
	{
	...
	let metadata_kind: NFTMetadataKind = utils
		::get_stored_value_with_user_errors::<u8>(
			NFT_METADATA_KIND,
			NFTCoreError::MissingNFTMetadataKind,
			NFTCoreError::InvalidNFTMetadataKind
	...
	let updated_token_count = match
		utils::get_dictionary_value_from_key::<u64>(TOKEN_COUNTS, &owned_tokens_item_key)
	{
	...
	if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
		...
		if
			NFTIdentifierMode::Hash == identifier_mode &&
		...
		let mut page_table = match
			storage
				::dictionary_get::<Vec<bool>>(page_table_uref, &owned_tokens_item_key)
				.unwrap_or_revert()
		{
		...
		let mut page = if !page_table[page_table_entry as usize] {
			...
			storage
				::dictionary_get::<Vec<bool>>(page_uref, &owned_tokens_item_key)
				.unwrap_or_revert()
...
pub extern "C" fn burn() {
	...
	let token_owner = match
		utils::get_dictionary_value_from_key::<Key>(
			TOKEN_OWNERS,
			&token_identifier.get_dictionary_item_key()
		)
	{
...
pub extern "C" fn approve() {
	...
	// revert.
	if
		let OwnershipMode::Minter | OwnershipMode::Assigned = utils
			::get_ownership_mode()
			.unwrap_or_revert()
	{
	...
	let token_owner_key = match
		utils::get_dictionary_value_from_key::<Key>(TOKEN_OWNERS, &token_identifier_dictionary_key)
	{
	...
	storage::dictionary_put(approved_uref, &token_identifier_dictionary_key, Some(operator));
}
...
pub extern "C" fn set_approval_for_all() {
	...
	let approve_all = utils
		::get_named_arg_with_user_errors::<bool>(
			ARG_APPROVE_ALL,
...
pub extern "C" fn transfer() {
	...
	let is_approved = match
		utils::get_dictionary_value_from_key::<Option<Key>>(
			OPERATOR,
			&token_identifier.get_dictionary_item_key()
		)
	{
	...
	runtime::print(
		&format!("caller: {}, owner: {}, isapproved: {}", &caller, &token_owner_key, is_approved)
	);
	...
	let target_owner_key: Key = utils
		::get_named_arg_with_user_errors(
			ARG_TARGET_KEY,
			NFTCoreError::MissingAccountHash,
			NFTCoreError::InvalidAccountHash
	...
	let updated_from_account_balance = match
		utils::get_dictionary_value_from_key::<u64>(TOKEN_COUNTS, &source_owner_item_key)
	{
		Some(balance) => {
			if balance > 0u64 {
				balance - 1u64
	...
	let updated_to_account_balance = match
		utils::get_dictionary_value_from_key::<u64>(TOKEN_COUNTS, &target_owner_item_key)
	{
		Some(balance) => balance + 1u64,
		None => 1u64,
	};
	...
	if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
		...
		let mut source_page = storage
			::dictionary_get::<Vec<bool>>(page_uref, &source_owner_item_key)
			.unwrap_or_revert()
		...
		let mut target_page_table = storage
			::dictionary_get::<Vec<bool>>(page_table_uref, &target_owner_item_key)
			.unwrap_or_revert()
		...
		let mut target_page = if !target_page_table[page_table_entry as usize] {
			...
			storage
				::dictionary_get::<Vec<bool>>(page_uref, &target_owner_item_key)
				.unwrap_or_revert()
				.unwrap_or_revert()
		};
		...
		let receipt = CLValue::from_t((receipt_string, owned_tokens_actual_key)).unwrap_or_revert_with(
			NFTCoreError::FailedToConvertToCLValue
		);
		runtime::ret(receipt)
	}
}
...
pub extern "C" fn balance_of() {
	...
	let balance = utils
		::get_dictionary_value_from_key::<u64>(TOKEN_COUNTS, &owner_key_item_string)
		.unwrap_or(0u64);

	let balance_cl_value = CLValue::from_t(balance).unwrap_or_revert_with(
		NFTCoreError::FailedToConvertToCLValue
	);
...
pub extern "C" fn owner_of() {
	...
	let token_owner_cl_value = CLValue::from_t(token_owner).unwrap_or_revert_with(
		NFTCoreError::FailedToConvertToCLValue
	);
...
pub extern "C" fn metadata() {
	...
	if let Some(metadata) = maybe_token_metadata {
		let metadata_cl_value = CLValue::from_t(metadata).unwrap_or_revert_with(
			NFTCoreError::FailedToConvertToCLValue
		);
...
pub extern "C" fn get_approved() {
	...
	let approved_cl_value = CLValue::from_t(maybe_approved).unwrap_or_revert_with(
		NFTCoreError::FailedToConvertToCLValue
	);
...
pub extern "C" fn set_token_metadata() {
	...
	let updated_token_metadata: String = utils
		::get_named_arg_with_user_errors::<String>(
			ARG_TOKEN_META_DATA,
			NFTCoreError::MissingTokenMetaData,
			NFTCoreError::InvalidTokenMetaData
	...
	let updated_metadata = metadata
		::validate_metadata(&metadata_kind, updated_token_metadata)
		.unwrap_or_revert();
...
pub extern "C" fn migrate() {
	match runtime::get_key(MIGRATION_FLAG) {
		Some(migration_key) => {
			let migration_uref = migration_key
				.into_uref()
				.unwrap_or_revert_with(NFTCoreError::InvalidKey);
			if storage::read::<bool>(migration_uref).unwrap_or_revert().unwrap_or_revert() {
	...
	let identifier: NFTIdentifierMode = utils
		::get_stored_value_with_user_errors::<u8>(
			IDENTIFIER_MODE,
			NFTCoreError::MissingIdentifierMode,
			NFTCoreError::InvalidIdentifierMode
...
pub extern "C" fn updated_receipts() {
	if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
		...
		let page_table = utils
			::get_dictionary_value_from_key::<Vec<bool>>(PAGE_TABLE, &token_owner_item_key)
			.unwrap_or_default();
...
pub extern "C" fn register_owner() {
	if let OwnerReverseLookupMode::Complete = utils::get_reporting_mode() {
		...
		if
			storage
				::dictionary_get::<Vec<bool>>(page_table_uref, &owner_item_key)
				.unwrap_or_revert()
				.is_none()
		{
		...
		let package_uref = storage::new_uref(
			utils::get_stored_value_with_user_errors::<String>(
				&format!("{}{}", CEP78_PREFIX, collection_name),
				NFTCoreError::MissingCep78PackageHash,
				NFTCoreError::InvalidCep78InvalidHash
			)
...
fn generate_entry_points() -> EntryPoints {
	...
	let init_contract = EntryPoint::new(
		...
		vec![
			...
			Parameter::new(ARG_CONTRACT_WHITELIST, CLType::List(Box::new(CLType::ByteArray(32u32)))),
	...
	let mint = EntryPoint::new(
		...
		CLType::Tuple3([Box::new(CLType::String), Box::new(CLType::Key), Box::new(CLType::String)]),
		EntryPointAccess::Public,
		EntryPointType::Contract
	);
	...
	let transfer = EntryPoint::new(
		ENTRY_POINT_TRANSFER,
		vec![Parameter::new(ARG_SOURCE_KEY, CLType::Key), Parameter::new(ARG_TARGET_KEY, CLType::Key)],
...
fn install_contract() {
	...
	let allow_minting: bool = utils
		::get_optional_named_arg_with_user_errors(ARG_ALLOW_MINTING, NFTCoreError::InvalidMintingStatus)
		.unwrap_or(true);
	...
	// This value cannot be changed after installation
	let whitelist_lock: u8 = utils
		::get_optional_named_arg_with_user_errors(
			ARG_WHITELIST_MODE,
			NFTCoreError::InvalidWhitelistMode
		)
		.unwrap_or(0u8);
	...
	// set to be unlocked.
	let contract_white_list: Vec<ContractHash> = utils
		::get_optional_named_arg_with_user_errors(
			ARG_CONTRACT_WHITELIST,
			NFTCoreError::InvalidContractWhitelist
		)
	...
	// This value cannot be changed after installation.
	let nft_metadata_kind: u8 = utils
		::get_named_arg_with_user_errors(
			ARG_NFT_METADATA_KIND,
			NFTCoreError::MissingNFTMetadataKind,
			NFTCoreError::InvalidNFTMetadataKind
	...
	// `src/modalities.rs` for further details.
	let metadata_mutability: u8 = utils
		::get_named_arg_with_user_errors(
			ARG_METADATA_MUTABILITY,
			NFTCoreError::MissingMetadataMutability,
			NFTCoreError::InvalidMetadataMutability
	...
	// `src/modalities.rs` for further details.
	let burn_mode: u8 = utils
		::get_optional_named_arg_with_user_errors(ARG_BURN_MODE, NFTCoreError::InvalidBurnMode)
		.unwrap_or(0u8);
	...
	runtime::put_key(&format!("{}{}", CONTRACT_NAME_PREFIX, collection_name), contract_hash.into());
	runtime::put_key(
		&format!("{}{}", CONTRACT_VERSION_PREFIX, collection_name),
		storage::new_uref(contract_version).into()
	);

	let package_hash: ContractPackageHash = runtime
		::get_key(&hash_key_name)
		.unwrap_or_revert()
		.into_hash()
...
fn migrate_contract(access_key_name: String, package_key_name: String) {
	let nft_contact_package_hash = runtime
		::get_key(&package_key_name)
		.unwrap_or_revert()
		.into_hash()
		.map(ContractPackageHash::new)
	...
	if let Some(access_key) = runtime::get_key(&access_key_name) {
		runtime::put_key(&format!("{}{}", ACCESS_KEY_NAME_PREFIX, collection_name), access_key);
	}
...
pub extern "C" fn call() {
	let convention_mode: NamedKeyConventionMode = utils
		::get_optional_named_arg_with_user_errors::<u8>(
			ARG_NAMED_KEY_CONVENTION,
	...
	match convention_mode {
		...
		NamedKeyConventionMode::V1_0Standard =>
			migrate_contract(ACCESS_KEY_NAME_1_0_0.to_string(), HASH_KEY_NAME_1_0_0.to_string()),
		NamedKeyConventionMode::V1_0Custom =>
			migrate_contract(
