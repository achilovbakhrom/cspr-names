...
use casper_types::{ bytesrepr, bytesrepr::{ FromBytes, ToBytes }, CLType, CLTyped };
...
pub(crate) fn get_metadata_schema(kind: &NFTMetadataKind) -> CustomMetadataSchema {
	match kind {
		...
		NFTMetadataKind::NFT721 => {
			...
			properties.insert("token_uri".to_string(), MetadataSchemaProperty {
		...
		NFTMetadataKind::CEP78 => {
			...
			properties.insert("checksum".to_string(), MetadataSchemaProperty {
...
impl FromBytes for CustomMetadataSchema {
	fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), bytesrepr::Error> {
		let (properties, remainder) = BTreeMap::<String, MetadataSchemaProperty>::from_bytes(bytes)?;
	...
	match metadata_kind {
		...
		NFTMetadataKind::NFT721 => {
			let metadata = casper_serde_json_wasm
				::from_str::<MetadataNFT721>(&token_metadata)
		...
		NFTMetadataKind::CustomValidated => {
			let custom_metadata = casper_serde_json_wasm
				::from_str::<BTreeMap<String, String>>(&token_metadata)
			...
			casper_serde_json_wasm
				::to_string_pretty(&custom_metadata.attributes)
