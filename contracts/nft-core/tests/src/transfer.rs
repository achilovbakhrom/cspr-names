...
use crate::utility::{
	...
	support::{
		...
		get_minting_contract_hash,
		get_minting_contract_package_hash,
...
fn should_transfer_token_from_sender_to_receiver() {
	...
	let actual_event: Transfer = support
...
fn approve_token_for_transfer_should_add_entry_to_approved_dictionary(
	...
	let spender = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
	...
	let actual_event: Approval = support
...
fn approve_token_for_transfer_from_an_account_should_add_entry_to_approved_dictionary() {
	let mut builder = InMemoryWasmTestBuilder::default();
	builder.run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST).commit();
	approve_token_for_transfer_should_add_entry_to_approved_dictionary(
		builder,
		None
	)
}
...
fn revoke_token_for_transfer_should_remove_entry_to_approved_dictionary(
	...
	let expected_event = ApprovalRevoked::new(
		owner_key,
		TokenIdentifier::Index(token_id)
	);
	let expected_event_index = if operator.is_some() { 3 } else { 2 };
	let actual_event: ApprovalRevoked = support
		::get_event(&builder, &nft_contract_key, expected_event_index)
		.unwrap();
	assert_eq!(actual_event, expected_event, "Expected ApprovalRevoked event.");
}
...
fn should_be_able_to_transfer_token(
	...
	let actual_approved_account: Option<Key> = get_dictionary_value_from_key(
		&builder,
		&nft_contract_key,
		APPROVED,
		&token_id.to_string()
	...
	let to_account = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_2)
	);
...
fn should_dissallow_same_approved_account_to_transfer_token_twice() {
	...
	let to_other_account = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_3)
	);
...
fn should_prevent_transfer_when_caller_is_not_owner() {
	...
	let other_account_secret_key = SecretKey::ed25519_from_bytes(
		[9u8; 32]
	).unwrap();
...
fn should_not_allow_non_approved_contract_to_transfer() {
	...
	let non_approved_transfer_request =
	...
	builder.exec(approved_transfer_request).expect_success().commit();
}
...
fn transfer_should_correctly_track_page_table_entries() {
	...
	let account_user_1_page_table =
		support::get_dictionary_value_from_key::<Vec<bool>>(
			&builder,
			&nft_contract_key,
			PAGE_TABLE,
...
fn should_prevent_transfer_to_unregistered_owner() {
	...
	assert_expected_error(
		error,
		128u16,
		"must raise unregistered owner in transfer"
	);
}
...
fn check_transfers_with_transfer_filter_contract_modes() {
	...
	let transfer_filter_contract_install_request =
	...
	let transfer_filter_contract_hash = get_transfer_filter_contract_hash(
		&builder
	);
...
fn should_disallow_transfer_from_contract_without_package_operator_mode_with_package_as_operator() {
	...
	let minting_contract_package_hash = get_minting_contract_package_hash(
		&builder
	);
...
fn should_disallow_package_operator_to_approve_without_package_operator_mode() {
	...
	let non_approved_approve_request =
		ExecuteRequestBuilder::contract_call_by_hash(
			*DEFAULT_ACCOUNT_ADDR,
			minting_contract_hash,
			ENTRY_POINT_APPROVE,
...
fn should_allow_package_operator_to_approve_with_package_operator_mode() {
	...
	builder.exec(approved_approve_request).expect_success().commit();
...
fn should_allow_package_operator_to_revoke_with_package_operator_mode() {
	...
	builder.exec(approved_revoke_request).expect_success().commit();
