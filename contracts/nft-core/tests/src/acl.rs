...
fn should_disallow_installation_with_contract_holder_mode_and_installer_mode() {
    ...
    let contract_whitelist = vec![Key::Hash([1u8; 32]), Key::Hash([2u8; 32]), Key::Hash([3u8; 32])];
...
fn should_disallow_unlisted_account_from_minting() {
    ...
    let error = builder.get_error().expect("should have an error");
    assert_expected_error(error, 36, "Unlisted account hash should not be permitted to mint");
}
...
fn should_disallow_unlisted_contract_from_minting() {
    ...
    let error = builder.get_error().expect("should have an error");
    assert_expected_error(error, 81, "Unlisted contract hash should not be permitted to mint");
}
...
fn should_disallow_unlisted_contract_from_minting_with_mixed_account_contract() {
    ...
    let minting_contract_hash = get_minting_contract_hash(&builder);
    let account_user_1 = support::create_funded_dummy_account(&mut builder, Some(ACCOUNT_USER_1));
    let mixed_whitelist = vec![Key::from(ContractHash::from([1u8; 32])), Key::from(account_user_1)];
...
fn should_disallow_unlisted_account_from_minting_with_mixed_account_contract() {
    ...
    let minting_contract_hash = get_minting_contract_hash(&builder);
    let mixed_whitelist = vec![Key::from(minting_contract_hash), Key::from(*DEFAULT_ACCOUNT_ADDR)];
...
fn should_disallow_contract_from_whitelisted_package_to_mint_without_acl_package_mode() {
    ...
    // println!("12345");
    assert!(is_whitelisted_contract_package, "acl whitelist is incorrectly set");
...
fn should_be_able_to_update_whitelist_for_minting_with_deprecated_arg_contract_whitelist() {
    ...
    let is_whitelisted_account = builder.query_dictionary_item(
        None,
        seed_uref,
        &minting_contract_hash.to_string()
    );

    assert!(is_whitelisted_account.is_err(), "acl whitelist is incorrectly set");
    ...
    builder.exec(update_whitelist_request).expect_success().commit();
...
fn should_upgrade_from_named_keys_to_dict_and_acl_minting_mode() {
    ...
    assert_eq!(minting_mode, MintingMode::Installer as u8, "minting mode should be set to public");
    ...
    assert_eq!(minting_mode, MintingMode::Acl as u8, "minting mode should be set to acl");
}
