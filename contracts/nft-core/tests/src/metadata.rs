...
fn should_prevent_update_in_immutable_mode() {
	...
	let update_token_metadata_request =
	...
	support::assert_expected_error(
		error,
		104,
		"must match ForbiddenMetadataUpdate(104)"
	)
}
...
fn should_prevent_install_with_hash_identifier_in_mutable_mode() {
	...
	assert_expected_error(
		error,
		102,
		"Should raise InvalidMetadataMutability(102)"
	)
}
...
fn should_allow_update_for_valid_metadata_based_on_kind(
	...
	let custom_metadata = serde_json
	...
	let token_hash = base16::encode_lower(
		&support::create_blake2b_hash(original_metadata)
	);

	let actual_metadata = match identifier_mode {
		NFTIdentifierMode::Ordinal =>
			support::get_dictionary_value_from_key::<String>(
				&builder,
				&nft_contract_key,
				dictionary_name,
				&(0u64).to_string()
	...
	let custom_updated_metadata = serde_json
		::to_string_pretty(&*TEST_CUSTOM_UPDATED_METADATA)
		.expect("must convert to json metadata");
	...
	let update_metadata_runtime_args = {
		...
		match identifier_mode {
			NFTIdentifierMode::Ordinal =>
				args.insert(ARG_TOKEN_ID, 0u64).expect("must get args"),
			NFTIdentifierMode::Hash =>
				args.insert(ARG_TOKEN_HASH, token_hash).expect("must get args"),
		}
		args
	};
	...
	let token_hash = base16::encode_lower(
		&support::create_blake2b_hash(updated_metadata)
	);

	builder.exec(update_metadata_request).expect_success().commit();
	...
	let actual_event: MetadataUpdated = support
		::get_event(&builder, &nft_contract_key, 1)
		.unwrap();
	assert_eq!(actual_event, expected_event, "Expected MetadataUpdated event.");
}
...
fn should_revert_minting_token_metadata_hash_twice() {
	...
	support::assert_expected_error(
		error,
		170,
		"can not mint twice the same token identifier"
	);
}
...
fn should_not_require_json_schema_when_kind_is(
	nft_metadata_kind: NFTMetadataKind
) {
	let mut builder = InMemoryWasmTestBuilder::default();
	builder.run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST).commit();
