...
fn should_burn_minted_token(reporting: OwnerReverseLookupMode) {
	...
	let reverse_lookup_enabled: bool =
		reporting == OwnerReverseLookupMode::Complete;
	...
	let actual_balance_before_burn =
		support::get_dictionary_value_from_key::<u64>(
			&builder,
			&nft_contract_key,
			TOKEN_COUNT,
...
fn should_return_expected_error_burning_of_others_users_token() {
	...
	support::assert_expected_error(
		error,
		6u16,
		"should disallow burning of other users' token"
	);
}
...
fn should_allow_contract_to_burn_token() {
	...
	builder.exec(burn_via_contract_call).expect_success().commit();
...
fn should_not_burn_in_non_burn_mode() {
	...
	support::assert_expected_error(
		error,
		106,
		"InvalidBurnMode(106) must have been raised"
	);
}
...
fn should_let_account_operator_burn_tokens_with_operator_burn_mode() {
	...
	let token_id = 0u64;
	let operator = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
	...
	let actual_event: Burn = support
		::get_event(&builder, &nft_contract_key, actual_event_index)
		.unwrap();
