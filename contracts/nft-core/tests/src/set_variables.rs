...
fn only_installer_should_be_able_to_toggle_allow_minting() {
	...
	let other_user_account = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
	...
	// Red test
	let other_user_set_variables_request =
	...
	// Red test
	builder.exec(other_user_set_variables_request).expect_failure().commit();
	...
	let allow_minting: bool = support::query_stored_value(
		&builder,
		nft_contract_key,
		vec![ALLOW_MINTING.to_string()]
	);
	...
	// Green test
	let installer_set_variables_request =
		ExecuteRequestBuilder::contract_call_by_hash(
			*DEFAULT_ACCOUNT_ADDR,
			nft_contract_hash,
			ENTRY_POINT_SET_VARIABLES,
	...
	builder.exec(installer_set_variables_request).expect_success().commit();
	...
	let actual_event: VariablesSet = support
