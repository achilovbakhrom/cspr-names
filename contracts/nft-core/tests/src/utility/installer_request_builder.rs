...
use nft_core_contract::constants::{
...
pub use nft_core_contract::modalities::{
	EventsMode,
	MintingMode,
	NFTHolderMode,
	NFTKind,
...
pub(crate) static TEST_CUSTOM_METADATA_SCHEMA: Lazy<CustomMetadataSchema> =
	Lazy::new(|| {
		let mut properties = BTreeMap::new();
		properties.insert("deity_name".to_string(), MetadataSchemaProperty {
			name: "deity_name".to_string(),
			description: "The name of deity from a particular pantheon.".to_string(),
			required: true,
		});
		properties.insert("mythology".to_string(), MetadataSchemaProperty {
...
pub(crate) static TEST_CUSTOM_METADATA: Lazy<
...
pub(crate) static TEST_CUSTOM_UPDATED_METADATA: Lazy<
	BTreeMap<String, String>
> = Lazy::new(|| {
	let mut attributes = BTreeMap::new();
	attributes.insert("deity_name".to_string(), "Baldur".to_string());
...
impl InstallerRequestBuilder {
	...
	pub(crate) fn default() -> Self {
		InstallerRequestBuilder {
			...
			collection_name: CLValue::from_t(NFT_TEST_COLLECTION.to_string()).expect(
				"name is legit CLValue"
			),
			collection_symbol: CLValue::from_t(NFT_TEST_SYMBOL).expect(
				"collection_symbol is legit CLValue"
			),
			total_token_supply: CLValue::from_t(1u64).expect(
				"total_token_supply is legit CLValue"
			),
			...
			json_schema: CLValue::from_t("test".to_string()).expect(
				"test_metadata was created from a concrete value"
			),
			nft_metadata_kind: CLValue::from_t(
				NFTMetadataKind::NFT721 as u8
			).unwrap(),
			identifier_mode: CLValue::from_t(
				NFTIdentifierMode::Ordinal as u8
			).unwrap(),
			metadata_mutability: CLValue::from_t(
				MetadataMutability::Mutable as u8
			).unwrap(),
			burn_mode: CLValue::from_t(BurnMode::Burnable as u8).unwrap(),
			operator_burn_mode: CLValue::from_t(false).unwrap(),
			reporting_mode: CLValue::from_t(
				OwnerReverseLookupMode::Complete as u8
			).unwrap(),
	...
	pub(crate) fn with_collection_name(
		mut self,
		collection_name: String
	) -> Self {
		self.collection_name = CLValue::from_t(collection_name).expect(
			"collection_name is legit CLValue"
		);
		self
	}

	pub(crate) fn with_invalid_collection_name(
		mut self,
		collection_name: CLValue
	) -> Self {
	...
	pub(crate) fn with_collection_symbol(
		mut self,
		collection_symbol: String
	) -> Self {
		self.collection_symbol = CLValue::from_t(collection_symbol).expect(
			"collection_symbol is legit CLValue"
		);
		self
	}

	pub(crate) fn with_invalid_collection_symbol(
		mut self,
		collection_symbol: CLValue
	) -> Self {
	...
	pub(crate) fn with_total_token_supply(
		mut self,
		total_token_supply: u64
	) -> Self {
		self.total_token_supply = CLValue::from_t(total_token_supply).expect(
			"total_token_supply is legit CLValue"
		);
		self
	}

	pub(crate) fn with_invalid_total_token_supply(
		mut self,
		total_token_supply: CLValue
	) -> Self {
	...
	pub(crate) fn with_allowing_minting(mut self, allow_minting: bool) -> Self {
		self.allow_minting = CLValue::from_t(allow_minting).expect(
			"allow minting is legit CLValue"
		);
		self
	}

	pub(crate) fn with_minting_mode(mut self, minting_mode: MintingMode) -> Self {
		self.minting_mode = CLValue::from_t(minting_mode as u8).expect(
			"public minting is legit CLValue"
		);
		self
	}

	pub(crate) fn with_ownership_mode(
		mut self,
	...
	pub(crate) fn with_whitelist_mode(
		mut self,
	...
	pub(crate) fn with_acl_package_mode(
		mut self,
		acl_package_mode: bool
	) -> Self {
		self.acl_package_mode = CLValue::from_t(acl_package_mode).expect(
			"acl package mode is legit CLValue"
		);
		self
	}

	pub(crate) fn with_package_operator_mode(
		mut self,
		package_operator_mode: bool
	) -> Self {
		self.package_operator_mode = CLValue::from_t(package_operator_mode).expect(
			"package operator mode is legit CLValue"
		);
		self
	}

	pub(crate) fn with_nft_metadata_kind(
		mut self,
	...
	pub(crate) fn with_additional_required_metadata(
		...
		self.additional_required_metadata = CLValue::from_t(
			Bytes::from(additional_required_metadata)
		).unwrap();
		self
	}

	pub(crate) fn with_optional_metadata(
		mut self,
		optional_metadata: Vec<u8>
	) -> Self {
		self.optional_metadata = CLValue::from_t(
			Bytes::from(optional_metadata)
		).unwrap();
		self
	}

	pub(crate) fn with_json_schema(mut self, json_schema: String) -> Self {
		self.json_schema = CLValue::from_t(json_schema).expect(
			"json_schema is legit CLValue"
		);
		self
	}

	pub(crate) fn with_identifier_mode(
		mut self,
	...
	pub(crate) fn with_metadata_mutability(
		...
		self.metadata_mutability = CLValue::from_t(
			metadata_mutability as u8
		).unwrap();
		self
	}
	...
	pub(crate) fn with_operator_burn_mode(
		mut self,
		operator_burn_mode: bool
	) -> Self {
		self.operator_burn_mode = CLValue::from_t(
			operator_burn_mode as u8
		).unwrap();
		self
	}

	pub(crate) fn with_reporting_mode(
		mut self,
	...
	pub(crate) fn with_transfer_filter_contract(
		mut self,
		transfer_filter_contract: Key
	) -> Self {
		self.transfer_filter_contract = Some(
			CLValue::from_t(transfer_filter_contract).unwrap()
		);
		self
	}
	...
	pub(crate) fn build(self) -> ExecuteRequest {
		...
		let json_schema = self.json_schema
			.clone()
			.into_t::<String>()
			.unwrap_or_default();
