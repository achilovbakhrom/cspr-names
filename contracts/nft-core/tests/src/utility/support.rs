use super::{
	constants::MINTING_CONTRACT_PACKAGE_NAME,
...
fn create_dummy_key_pair(account_string: [u8; 32]) -> (SecretKey, PublicKey) {
	let secret_key = SecretKey::ed25519_from_bytes(account_string).expect(
...
pub(crate) fn create_funded_dummy_account(
	...
	let (_, account_public_key) = create_dummy_key_pair(
		if let Some(account_string) = account_string {
			account_string
		} else {
			[7u8; 32]
		}
...
pub(crate) fn assert_expected_invalid_installer_request(
	...
	builder.exec(install_request_builder.build()).expect_failure(); // Should test against expected error
...
pub(crate) fn call_session_code_with_ret<T: CLTyped + FromBytes>(
	...
	runtime_args.insert(ARG_NFT_CONTRACT_HASH, nft_contract_key).unwrap();
	runtime_args.insert(ARG_KEY_NAME, key_name.to_string()).unwrap();
...
pub(crate) fn create_blake2b_hash<T: AsRef<[u8]>>(
	data: T
) -> [u8; BLAKE2B_DIGEST_LENGTH] {
	let mut result = [0; BLAKE2B_DIGEST_LENGTH];
	// NOTE: Assumed safe as `BLAKE2B_DIGEST_LENGTH` is a valid value for a hasher
	let mut hasher = VarBlake2b::new(BLAKE2B_DIGEST_LENGTH).expect(
		"should create hasher"
	);
...
fn make_page_dictionary_item_key(token_owner_key: &Key) -> String {
	match token_owner_key {
		Key::Account(token_owner_account_hash) =>
			token_owner_account_hash.to_string(),
		Key::Hash(token_owner_hash_addr) =>
			ContractHash::new(*token_owner_hash_addr).to_string(),
...
pub(crate) fn get_token_page_by_hash(
	...
	let token_number: u64 = get_dictionary_value_from_key(
		builder,
		nft_contract_key,
		INDEX_BY_HASH,
		&token_hash
