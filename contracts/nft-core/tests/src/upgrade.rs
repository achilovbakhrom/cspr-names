...
use nft_core_contract::{
	constants::{
		...
		PAGE_LIMIT,
		PREFIX_ACCESS_KEY_NAME,
		PREFIX_HASH_KEY_NAME,
		RECEIPT_NAME,
...
fn should_safely_upgrade_in_ordinal_identifier_mode() {
	...
	let previous_token_representation =
		support::get_dictionary_value_from_key::<Vec<u64>>(
			&builder,
	...
	let actual_event: Migration = support
...
fn should_safely_upgrade_in_hash_identifier_mode() {
	...
	for i in 0..number_of_tokens_pre_migration {
		...
		let json_token_metadata = serde_json
			::to_string_pretty(&token_metadata)
			.expect("must convert to string");

		let token_hash = base16::encode_lower(
			&support::create_blake2b_hash(&json_token_metadata)
		);
	...
	let previous_token_representation =
		support::get_dictionary_value_from_key::<Vec<String>>(
			&builder,
			&nft_contract_key_1_0_0,
			OWNED_TOKENS,
			&DEFAULT_ACCOUNT_ADDR.clone().to_string()
	...
	let number_of_tokens_at_upgrade = support
		::get_stored_value_from_global_state::<u64>(
			&builder,
			nft_contract_key,
	...
	let total_token_supply_post_upgrade = support
		::get_stored_value_from_global_state::<u64>(
			&builder,
			nft_contract_key,
	...
	builder.exec(post_upgrade_mint_request).expect_success().commit();
...
fn should_update_receipts_post_upgrade_paged() {
	...
	let nft_receipt: String = support
		::get_stored_value_from_global_state(
			&builder,
			nft_contract_key,
	...
	let actual_page_0 = support
		::get_stored_value_from_global_state::<Vec<bool>>(
			&builder,
			receipt_page_0,
			vec![]
		)
...
fn should_not_be_able_to_reinvoke_migrate_entrypoint() {
	...
	support::assert_expected_error(
		error,
		126u16,
		"must have previously migrated error"
	);
}
...
fn should_not_migrate_contracts_with_zero_token_issuance() {
	...
	support::assert_expected_error(
		error,
		122u16,
		"cannot upgrade when issuance is 0"
	);
}
...
fn should_upgrade_with_custom_named_keys() {
	...
	let mangle_named_keys_request = ExecuteRequestBuilder::standard(
		*DEFAULT_ACCOUNT_ADDR,
		MANGLE_NAMED_KEYS,
		runtime_args! {}
	).build();

	builder.exec(mangle_named_keys_request).expect_success().commit();
	...
	builder.exec(proper_upgrade_request).expect_success().commit();
}
...
fn should_safely_upgrade_from_old_version_to_new_version_with_reporting_mode(
	...
	let nft_contract_hash: ContractHash = support::get_nft_contract_hash(
		&builder
	);
	let nft_contract_key: Key = nft_contract_hash.into();
...
fn should_safely_upgrade_from_1_0_0_to_1_2_0_to_current_version() {
	...
	let nft_contract_hash_1_2_0: ContractHash = support::get_nft_contract_hash(
		&builder
	);
...
fn should_safely_upgrade_from_1_0_0_to_1_3_0_to_current_version() {
	...
	let nft_contract_hash_1_3_0: ContractHash = support::get_nft_contract_hash(
		&builder
	);
