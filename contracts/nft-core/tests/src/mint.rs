...
use crate::utility::{
	...
	support::{
		...
		call_session_code_with_ret,
		create_funded_dummy_account,
...
fn setup_nft_contract(
	...
	let mut install_request_builder = InstallerRequestBuilder::new(
		*DEFAULT_ACCOUNT_ADDR,
		NFT_CONTRACT_WASM
	)
	...
	builder.exec(install_request_builder.build()).expect_success().commit();
	builder
}
...
fn entry_points_with_ret_should_return_correct_value() {
	...
	let expected_approved_account = Key::Account(
		AccountHash::new(ACCOUNT_USER_1)
	);
	assert_eq!(
...
fn should_mint() {
	...
	let install_request_builder = InstallerRequestBuilder::new(
		*DEFAULT_ACCOUNT_ADDR,
		NFT_CONTRACT_WASM
	)
		.with_ownership_mode(OwnershipMode::Transferable)
	...
	let actual_event: Mint = support
		::get_event(&builder, &nft_contract_key, 0)
		.unwrap();
...
fn mint_should_increment_number_of_minted_tokens_by_one_and_add_public_key_to_token_owners() {
	...
	let minter_account_hash = support
		::get_dictionary_value_from_key::<Key>(
			&builder,
			&nft_contract_key,
			TOKEN_OWNERS,
			&token_id.to_string()
...
fn should_set_issuer() {
	...
	let actual_token_issuer = support
		::get_dictionary_value_from_key::<Key>(
			&builder,
			&nft_contract_key,
			TOKEN_ISSUERS,
...
fn should_set_issuer_with_different_owner() {
	...
	let account_user_1 = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
...
fn should_allow_minting_for_different_public_key_with_minting_mode_set_to_public() {
	...
	let account_user_2 = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_2)
	);
...
fn should_set_approval_for_all() {
	...
	builder.exec(set_approve_for_all_request).expect_success().commit();
	...
	let actual_event: ApprovalForAll = support
		::get_event(&builder, &nft_contract_key, 1)
		.unwrap();
	assert_eq!(actual_event, expected_event, "Expected ApprovalForAll event.");

	// Test if two minted tokens are transferable by operator
	let token_receiver = support::create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
	...
	let actual_token_owner = support
		::get_dictionary_value_from_key::<Key>(
			&builder,
			&nft_contract_key,
			TOKEN_OWNERS,
...
fn should_revoke_approval_for_all() {
	...
	let revoke_approve_for_all_request =
		ExecuteRequestBuilder::contract_call_by_name(
			*DEFAULT_ACCOUNT_ADDR,
			CONTRACT_NAME,
			ENTRY_POINT_SET_APPROVALL_FOR_ALL,
	...
	builder.exec(revoke_approve_for_all_request).expect_success().commit();
	...
	let actual_event: RevokedForAll = support
		::get_event(&builder, &nft_contract_key, 2)
		.unwrap();
	assert_eq!(actual_event, expected_event, "Expected RevokedForAll event.");
}
...
fn should_mint_with_compactified_metadata() {
	...
	builder.exec(install_request_builder).expect_success().commit();
...
fn should_mint_with_custom_metadata_validation() {
	...
	let custom_json_schema = serde_json
		::to_string(&*TEST_CUSTOM_METADATA_SCHEMA)
	...
	let pretty_custom_metadata = serde_json
		::to_string_pretty(&*TEST_CUSTOM_METADATA)
...
fn should_mint_with_hash_identifier_mode() {
	...
	let token_id_hash: String = base16::encode_lower(
		&support::create_blake2b_hash(TEST_PRETTY_721_META_DATA)
	);
...
fn should_fail_to_mint_when_immediate_caller_is_account_in_contract_mode() {
	...
	assert_expected_error(
		error,
		76,
		"InvalidHolderMode(76) must have been raised"
	);
}
...
fn should_approve_in_hash_identifier_mode() {
	...
	let token_hash: String = base16::encode_lower(
		&support::create_blake2b_hash(TEST_PRETTY_721_META_DATA)
	);
	...
	let maybe_approved_account =
		support::get_dictionary_value_from_key::<Option<Key>>(
			&builder,
			&nft_contract_key,
			APPROVED,
			&token_hash
...
fn should_approve_all_in_hash_identifier_mode() {
	...
	let actual_event: ApprovalForAll = support
		::get_event(&builder, &nft_contract_key, expected_event_index)
		.unwrap();
	assert_eq!(actual_event, expected_event, "Expected ApprovalForAll event.");
}
...
fn should_approve_all_with_flat_gas_cost() {
	...
	let operator = create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_1)
	);
	let operator_key = Key::Account(operator);
	...
	let other_operator = create_funded_dummy_account(
		&mut builder,
		Some(ACCOUNT_USER_2)
	);
	...
	assert!(is_also_operator, "expected other operator to be approved for all");
