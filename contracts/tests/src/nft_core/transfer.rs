...
fn approve_token_for_transfer_should_add_entry_to_approved_dictionary() {
	...
	assert_eq!(actual_approved_key, Some(Key::Account(approve_public_key.to_account_hash())));
}
...
fn should_be_able_to_transfer_token_using_approved_operator() {
	...
	builder.exec(transfer_to_to_account).expect_success().commit();
	...
	assert_eq!(actual_approved_account_hash, None, "operator should be set to none after a transfer");
}
...
fn should_prevent_transfer_when_caller_is_not_owner() {
	...
	assert_expected_error(error, 6u16, "transfer from another account must raise InvalidTokenOwner");
}
