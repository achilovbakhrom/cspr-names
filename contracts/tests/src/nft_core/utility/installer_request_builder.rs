...
impl InstallerRequestBuilder {
	pub(crate) fn new(account_hash: AccountHash, session_file: &str) -> Self {
		Self::default().with_account_hash(account_hash).with_session_file(session_file.to_string())
	}
	...
	pub(crate) fn with_named_key_convention_mode(
		mut self,
		named_key: NamedKeyConventionMode
	) -> Self {
		self.named_key_convention = CLValue::from_t(named_key as u8).unwrap();
		self
	}
	...
	pub(crate) fn with_allowing_minting(mut self, allow_minting: bool) -> Self {
		self.allow_minting = CLValue::from_t(allow_minting).expect("allow minting is legit CLValue");
		self
	}
