...
pub fn account_has_access_for_contract(operators_inclusively: bool) -> bool {
	let caller = match get_verified_caller(CallerVerificationType::OnlyAccountHash) {
		Ok(res) => res.into_account().unwrap_or_revert_with(CommonError::InvalidCaller),
		Err(e) => {
	...
	let maintainer = get_stored_value_from_key(KEY_MAINTAINER).unwrap_or_revert_with(
		CommonError::MissingMaintainer
	);
	...
	if operators_inclusively {
		let registry_contract_hash = get_stored_value_from_key(
			KEY_REGISTRY_CONTRACT_HASH
		).unwrap_or_revert_with(CommonError::MissingRegistryHash);
		let current_contract_hash: ContractHash = get_stored_value_from_key(
			KEY_CONTRACT_HASH
