...
pub fn create_new_contract(entrypoints: EntryPoints, mut named_keys: BTreeMap<String, Key>) {
    let maintainer_uref = storage::new_uref(runtime::get_caller());
    named_keys.insert(CONTRACT_MAINTAINER_KEY.to_string(), maintainer_uref.into());
...
pub fn is_maintainer(account: &AccountHash) -> bool {
    let maintainer = get_stored_value_from_key::<AccountHash>(CONTRACT_MAINTAINER_KEY)
