...
use common_lib::constants::common_keys::{
	...
	KEY_NFT_CONTRACT_HASH,
	KEY_NFT_CONTRACT_OWNER,
	KEY_NFT_CONTRACT_PACKAGE_NAME,
...
pub extern "C" fn set_nft_contract_hash() {
	let contract_hash = runtime::get_named_arg::<ContractHash>(
		ARG_NFT_CONTRACT_HASH
	);
...
pub extern "C" fn call() {
	...
	entrypoints.add_entry_point(
		EntryPoint::new(
			ENDPOINT_NFT_SET_NFT_CONTRACT_HASH,
			vec![Parameter::new(ARG_NFT_CONTRACT_HASH, ContractHash::cl_type())],
			CLType::Unit,
			EntryPointAccess::Public,
			EntryPointType::Contract
		)
	...
	nft_named_keys.insert(
		KEY_NFT_CONTRACT_OWNER.to_string(),
		owner_value_uref.into()
	);
	let version = 1;
