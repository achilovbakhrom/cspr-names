...
use common_lib::{
    constants::{
        ARG_AUTHORITY_CONTRACT_TYPE, ARG_AUTHORITY_EXTENSION, ARG_DATABASE_DOMAIN_NAME,
        ARG_DATABASE_RESOLVER, ARG_MAIN_AUTHORITY, ARG_MAIN_CUSTOMER_PURSE, ARG_MAIN_DOMAIN,
        ARG_MAIN_DOMAIN_PAGE, ARG_MAIN_DURATION, ARG_MAIN_PRICE_ORACLE_CONTRACT_HASH,
        ARG_MAIN_REGISTER_AMOUNT, ARG_MAIN_RESOLVER_ADDRESS, ARG_MAIN_SUBDOMAIN, ARG_NFT_METADATA,
        ARG_NFT_TOKEN_OWNER, ARG_PO_PRICE_TYPE_CHARS_COUNT, ENDPOINT_AUTHORITY_GET_CONTRACT,
        ENDPOINT_DATABASE_GET_DOMAIN, ENDPOINT_DATABASE_SAVE_DOMAIN_NAME, ENDPOINT_PO_GET_PRICE,
        ENTRYPOINT_MAIN_ADD_AUTHORITY, ENTRYPOINT_MAIN_EXTEND,
        ENTRYPOINT_MAIN_GET_SUBDOMAINS_FOR_DOMAIN, ENTRYPOINT_MAIN_REGISTER_DOMAIN,
        ENTRYPOINT_MAIN_REGISTER_SUB_DOMAIN, ENTRYPOINT_MAIN_REMOVE_AUTHORITY,
        ENTRYPOINT_MAIN_REMOVE_SUBDOMAIN, ENTRYPOINT_MAIN_RESOLVE_DOMAIN,
    ...
    enums::{
        contracts_enum::ContractKind, domain_name_actual_state::DomainNameActualState,
        main_contract::Error,
    },
...
pub extern "C" fn register_domain() {
    ...
    // Validation
    let validator = NamesValidator::instance(extensions.unwrap(), vec![maintainer]);
    ...
    if authorities_hash.is_none() {
        return response_error(MainContractErrors::AuthoritiesContractHashIsNotSet);
    }
    ...
    let purse: URef = get_stored_value_from_key(KEY_MAIN_MAINTAINER_PURSE)
        .unwrap_or_revert_with(MainContractErrors::MaintainerPurseIsNotSet);
    ...
    let db_contract_hash = match get_contract_hash_from_authority_contract(
        ...
        Ok(hash) => match hash {
            Some(res) => res,
            None => return response_error(MainContractErrors::DatabaseFulfilledOrIsNotSet),
        },
        Err(e) => return response_error(e),
    };
...
pub extern "C" fn resolve_domain() {
    ...
    let db_contract_hash =
        match NameContractHashDb::instance().get_contract_hash_for_domain_name(&domain_name) {
            Some(res) => res,
            None => return response_error(MainContractErrors::DomainNotExists),
        };
