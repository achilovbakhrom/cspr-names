...
pub struct NamesValidator {
    ...
    allowed_accounts: Vec<AccountHash>,
}
...
impl NamesValidator {
    ...
    pub fn validate_name(&self, arg: String) -> Result<NamesModel, MainContractErrors> {
        ...
        if model.name.len() <= 3 && !self.allowed_accounts.contains(&caller) {
...
impl From<String> for NamesModel {
    ...
    fn from(value: String) -> Self {
        let split = value.split(".").map(|item| item.to_string()).collect::<Vec<String>>();
        ...
        if split.len() == 2 {
            ...
            name = split.get(split.len() - 2).unwrap().to_string();
            let sub_name_vec = split.iter().take(split.len() - 2).map(|item| item.to_string()).collect::<Vec<String>>();
